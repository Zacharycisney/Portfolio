...
recordRoutes.route("/record/find").post(async (req, res) => {
    try {
        let db_connect = dbo.getDb();
        const user = await db_connect.collection("records").findOne({
        ...
        if (user) {
            ...
            console.log("User session set:", req.session.user);  // Log session data
...
recordRoutes.route("/record/add").post(async (req, res) => {
    try {
        let db_connect = dbo.getDb();
        const existingUser = await db_connect.collection("records").findOne({ email: req.body.email });
...
recordRoutes.route("/logout").post((req, res) => {
    req.session.destroy();
...
recordRoutes.route("/current_user").get(async (req, res) => {
    try {
        if (!req.session.user) {
        ...
        let db_connect = dbo.getDb();
        let user = await db_connect.collection("records").findOne({ _id: new ObjectId(req.session.user.id) }, { projection: { password: 0 } });

        if (user) {
            console.log("Current user fetched:", user);  // Log fetched user data
            res.json(user);
        } else {
            res.status(404).json({ error: "User not found" });
        }
...
recordRoutes.route("/deposit").post(async (req, res) => {
    try {
        ...
        let myquery = { email: req.session.user.email };
        ...
        let { accountType, amount } = req.body;
        ...
        let newBalance = (user[accountType] || 0) + amount;
...
recordRoutes.route("/withdraw").post(async (req, res) => {
    try {
        ...
        let currentBalance = user[accountType] || 0;
